...
def _image_b64(image: Image.Image, image_format: str = "png", jpeg_quality: int = 95) -> tuple[str, str]:
    image_format = image_format.lower()
    formats = {"png": ("PNG", "image/png"), "jpeg": ("JPEG", "image/jpeg"), "jpg": ("JPEG", "image/jpeg"), "webp": ("WEBP", "image/webp")}
    if image_format not in formats:
        raise ValueError(f"Unsupported image_format: {image_format!r}")
    pil_format, media_type = formats[image_format]
    buf = BytesIO()
    kwargs = {"quality": int(jpeg_quality)} if pil_format in {"JPEG", "WEBP"} else {}
    image.convert("RGB").save(buf, pil_format, **kwargs)
    return b64encode(buf.getvalue()).decode("ascii"), media_type
...
class HTTPVictim:
    ...
    def __init__(self, config: dict[str, Any], transport: Callable | None = None):
        ...
        self.api_key_env = str(config.get("api_key_env") or self.default_api_key_env)
    ...
    def _encoded_image(self, image: Image.Image) -> tuple[str, str]:
        return _image_b64(
            image,
            str(self.config.get("image_format", "png")),
            int(self.config.get("jpeg_quality", 95)),
        )

    def _headers(self, headers: dict[str, str]) -> dict[str, str]:
        if self.config.get("browser_headers"):
            headers = {
...
class OpenAIVictim(HTTPVictim):
    ...
    def _request(self, image, prompt):
        image_data, media_type = self._encoded_image(image)
        image_url: dict[str, Any] = {"url": f"data:{media_type};base64,{image_data}"}
        if self.config.get("image_detail") is not None:
            image_url["detail"] = str(self.config["image_detail"])
        ...
        raw = self._transport(f"{self.base_url}/chat/completions", self._headers({"Authorization": f"Bearer {self._api_key()}", "Content-Type": "application/json"}), payload, self.timeout)
...
class GeminiVictim(HTTPVictim):
    ...
    def _request(self, image, prompt):
        ...
        payload = {"contents": [{"role": "user", "parts": [{"text": prompt}, {"inline_data": {"mime_type": media_type, "data": image_data}}]}]}
        if generation:
            payload["generationConfig"] = generation
        raw = self._transport(f"{self.base_url}/models/{self.model_id}:generateContent?key={self._api_key()}", self._headers({"Content-Type": "application/json"}), payload, self.timeout)
...
class AnthropicVictim(HTTPVictim):
    ...
    def _request(self, image, prompt):
        ...
        for key in ("temperature", "top_p", "top_k"):
            if self.config.get(key) is not None:
                payload[key] = self.config[key]
        raw = self._transport(f"{self.base_url}/messages", self._headers({"x-api-key": self._api_key(), "anthropic-version": "2023-06-01", "Content-Type": "application/json"}), payload, self.timeout)
