...
PYTHONPATH="$ROOT/src:$ROOT" TECHUTOPIA_API_KEY="${TECHUTOPIA_API_KEY:-research-cycle}" \
  ...
  --hours 8 --gpu-jobs 6 --heldout-batch-size 48 --augmentation-workers 2 --promote 3 --run-api \
  --output "$OUTPUT" \
...
if (( CYCLE_EXIT != 0 )); then
  printf '{"cycle_exit": %s, "confirmation_exit": null, "confirmation_skipped": true}\n' \
    "$CYCLE_EXIT" >"$OUTPUT/overall_exit.json"
  exit "$CYCLE_EXIT"
fi
...
PYTHONPATH="$ROOT/src:$ROOT" \
  ...
  --cycle-root "$OUTPUT" --batch-size 64 --attack-workers 2 --run-api --api-limit 50 \
