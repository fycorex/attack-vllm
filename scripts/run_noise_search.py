...
def trial_id(value: dict) -> str:
    readable = f"{value['dataset']}__{value['surrogate_group']}__{value['mode']}__s{value['sigma']:.5f}__n{value['samples']}__seed{value['seed']}"
...
def build_config(spec: dict, trial: dict, output: Path) -> tuple[Path, dict]:
    ...
    return path, {"dataset": dataset, "group": group}
...
def execute(spec: dict, trial: dict, root: Path, python: str) -> dict:
    ...
    heldout = json.loads(heldout_path.read_text()); result = {**trial, "trial_id": trial_id(trial), "heldout_macro_asr": heldout["heldout_macro_asr"],
...
def main() -> None:
    ...
    parser.add_argument("--mode"); parser.add_argument("--dataset"); parser.add_argument("--surrogate-group")
